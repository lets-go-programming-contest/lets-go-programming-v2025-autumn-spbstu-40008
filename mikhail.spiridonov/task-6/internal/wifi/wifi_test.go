...
func (m *mockWiFiHandle) Interfaces() ([]*extwifi.Interface, error) {
	...
	if args.Get(0) == nil {
		return nil, fmt.Errorf("get interfaces: %w", args.Error(1))
	}

	ifaces, ok := args.Get(0).([]*extwifi.Interface)
	if !ok {
		return nil, fmt.Errorf("type assertion: %w", errTypeAssertionInterfaces)
	}

	err := args.Error(1)
	if err != nil {
		return ifaces, fmt.Errorf("interfaces error: %w", err)
	}
...
func (m *mockWiFiHandle) StationInfo(ifi *extwifi.Interface) (*extwifi.StationInfo, error) {
	...
	if args.Get(0) == nil {
		return nil, fmt.Errorf("get station info: %w", args.Error(1))
	}

	info, ok := args.Get(0).(*extwifi.StationInfo)
	if !ok {
		return nil, fmt.Errorf("type assertion: %w", errTypeAssertionStation)
	}

	err := args.Error(1)
	if err != nil {
		return info, fmt.Errorf("station info error: %w", err)
	}
